...
def aggregate_results(data: pd.DataFrame, results: Iterable[dict]) -> pd.DataFrame:
    ...
    # Signed aggregate error: positive means the simulator overpredicts q.
    # y_log_abs_error is preserved as the historical mean per-seed absolute log error.
    out["y_signed_log_error"] = np.log1p(out["q_pred_mean"]) - np.log1p(out["q_true"])
