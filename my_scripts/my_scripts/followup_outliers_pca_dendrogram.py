...
IN = r"E:\JupyterPjs\MOF_SynBench\ActMOF_Workshop\Error_data.csv"
...
df.to_csv(os.path.join(OUTDIR, "Error_data_with_outlier_flags.csv"), index=False)
...
fig.tight_layout()
fig.savefig(os.path.join(OUTDIR, "pca_2d.png"), dpi=170)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(os.path.join(OUTDIR, "feature_dendrogram.png"), dpi=170)
plt.close(fig)
...
def binned_catastrophic_heatmap(xcol, ycol, xbins, ybins, xlabel, ylabel, fname, xlabels=None, ylabels=None):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(OUTDIR, fname), dpi=170)
    plt.close(fig)
