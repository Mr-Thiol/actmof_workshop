...
OUTDIR = r"E:\JupyterPjs\MOF_SynBench\ActMOF_Workshop\figures\followup"
err = pd.read_csv(r"E:\JupyterPjs\MOF_SynBench\ActMOF_Workshop\Error_data.csv")
cont = pd.read_csv(os.path.join(OUTDIR, "continuous_features.csv"))
...
fig.tight_layout()
fig.savefig(os.path.join(OUTDIR, "continuous_margins_MI_MF.png"), dpi=170)
plt.close(fig)
...
dens_corr.to_csv(os.path.join(OUTDIR, "density_feature_correlations.csv"), index=False)
...
fig.tight_layout()
fig.savefig(os.path.join(OUTDIR, "density_vs_error.png"), dpi=170)
plt.close(fig)
